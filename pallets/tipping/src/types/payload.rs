...
#[derive(Encode, Decode, Clone, Default, RuntimeDebug, PartialEq, Eq, TypeInfo)]
pub struct IndexingData<AccountId>(Vec<u8>, Payload<AccountId>, Option<DataType>, Option<Id>);
impl<AccountId> IndexingData<AccountId> {
	...
	pub fn get_data_id(&self) -> &Option<Id> {
		&self.3
	}

	pub fn init(key_id: &[u8], payload: Payload<AccountId>) -> Self {
		Self(key_id.to_vec(), payload, None, None)
	}

	pub fn set_key_id(mut self, key_id: &[u8]) -> Self {
		self.0 = key_id.to_vec();
		self
	}

	pub fn set_payload(mut self, payload: Payload<AccountId>) -> Self {
		self.1 = payload;
		self
	}

	pub fn set_data_type(mut self, data_type: &DataType) -> Self {
		self.2 = Some(data_type.clone());
		self
	}

	pub fn set_data_id(mut self, data_id: &[u8]) -> Self {
		self.3 = Some(data_id.to_vec());
		self
	}
}
