...
pub mod pallet {
	...
	pub(super) type Credentials<T: Config> =
		StorageMap<_, Blake2_128Concat, AccountIdOf<T>, UserCredential<HashOf<T>, AccountIdOf<T>>>;

	#[pallet::storage]
	#[pallet::getter(fn credential)]
	pub(super) type Credential<T: Config> =
		StorageMap<_, Blake2_128Concat, PeopleId, UserCredentialInfo<AccountIdOf<T>>>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn remove_credential(
			...
			match Self::delete_credential(&destroyer, &people_id) {
				Ok(people_id) => {
					Self::deposit_event(Event::CredentialRemoved(people_id));
					Self::deposit_event(Event::UserCredentialsUpdated(destroyer));
	...
	impl<T: Config> Pallet<T> {
		pub fn create_credential(
			...
			let found_platform =
				platforms.clone().into_iter().find(|el| el == &credential_info.platform);
			...
			if credential_owner.is_none() {
				...
				let credential_owner = credential_owner.unwrap_or_default();
				...
				if credential_owner
					.credentials
					.iter()
					.any(|credential| credential.platform == credential_info.platform)
				{
					return Err(Error::<T>::CredentialAlreadyExist)
				}
				...
				credential_id =
					Self::generate_credential_id(owner_id, credential_info, &credential_count);
				credentials = credential_owner.credentials;
				credentials.push(new_credential_info.clone());
			}

			new_credential =
				UserCredential { id: credential_id, owner_id: owner_id.clone(), credentials };
		...
		pub fn delete_credential(
			...
			let found_credential =
				credentials.iter().find(|credential| credential.people_id == people_id);
