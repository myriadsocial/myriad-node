...
pub mod pallet {
	...
	pub(super) type Token<T: Config> =
		StorageMap<_, Blake2_128Concat, TokenId, TokenDetail<T::Hash>>;
	...
	impl<T: Config> Pallet<T> {
		pub fn create_token(
			...
			token_info: &TokenInfo,
		) -> Result<TokenDetail<HashOf<T>>, Error<T>> {
			...
			if token.is_some() {
				return Err(Error::<T>::TokenAlreadyExist)
			}

			let new_token = TokenDetail { id: generate_id, token_info: token_info.clone() };
		...
		pub fn delete_token(token_id: &[u8]) -> Result<(), Error<T>> {
			let tokens: Vec<TokenInfo> = Self::tokens().unwrap_or_default();

			let found_token = tokens.iter().find(|token| token.token_id == token_id);

			if found_token.is_none() {
				return Err(Error::<T>::TokenNotExist)
			}

			let updated_tokens: Vec<TokenInfo> =
				tokens.into_iter().filter(|token| token.token_id != token_id).collect();

			Token::<T>::remove(token_id);
